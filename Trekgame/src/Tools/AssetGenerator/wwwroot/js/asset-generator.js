// Asset Generator JavaScript Functions
...
window.cropToSize = function(base64Data, targetWidth, targetHeight) {
    return new Promise((resolve, reject) => {
        const img = new Image();
        img.onload = function() {
            console.log(`cropToSize: Source ${img.width}x${img.height} → Target ${targetWidth}x${targetHeight}`);
...
// Auto-detect grid from image dimensions (assumes square cells of standard sizes)
window.detectGridFromImage = function(base64Image, expectedColumns) {
    return new Promise((resolve, reject) => {
        const img = new Image();
        img.onload = () => {
            ...
            // Common cell sizes to try (in order of preference)
            const commonCellSizes = [512, 256, 384, 320, 128, 448, 576, 640];

            let bestMatch = null;

            // First, try to detect based on expectedColumns if provided
            if (expectedColumns && expectedColumns > 0) {
                const cellWidth = Math.floor(width / expectedColumns);
                // Check if this gives us integer rows
                if (height % cellWidth === 0 || Math.abs(height / cellWidth - Math.round(height / cellWidth)) < 0.1) {
                    const rows = Math.round(height / cellWidth);
                    bestMatch = { columns: expectedColumns, rows: rows, cellSize: cellWidth };
                }
            }

            // If no match yet, try common cell sizes
            if (!bestMatch) {
                for (const cellSize of commonCellSizes) {
            ...
            if (!bestMatch) {
                for (const cellSize of commonCellSizes) {
                    ...
                    if (cols > 0 && rows > 0) {
                        ...
                        // Accept if cells are roughly square
                        if (Math.abs(actualCellWidth - actualCellHeight) < 20) {
                            bestMatch = {
                                columns: cols,
                                rows: rows,
                                cellSize: Math.floor(actualCellWidth)
                            };
                            break;
                        }
                    }
...
// Split spritesheet into individual images
window.splitSpritesheet = function(base64Image, columns, rows) {
    return new Promise((resolve, reject) => {
        const img = new Image();
        img.onload = () => {
            ...
            for (let row = 0; row < rows; row++) {
                for (let col = 0; col < columns; col++) {
                    ...
                    // Extract this cell from the spritesheet
                    ctx.drawImage(
                        img,
                        col * cellWidth, row * cellHeight,  // Source position
                        cellWidth, cellHeight,               // Source size
                        0, 0,                                // Destination position
                        cellWidth, cellHeight                // Destination size
                    );

                    // Check if cell has content (not entirely black/transparent)
                    ...
                    for (let i = 0; i < imageData.data.length; i += 4) {
                        ...
                        // Check if pixel is visible (not black or transparent)
        ...
        img.onerror = (e) => {
            console.error('splitSpritesheet: Failed to load image', e);
            reject(e);
        };
        img.src = 'data:image/png;base64,' + base64Image;
    });
